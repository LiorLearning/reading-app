...
import { playImageLoadingSound, stopImageLoadingSound, playImageCompleteSound, playMessageSound } from "@/lib/sounds";
...
const Index = () => {
  ...
  const onGenerate = useCallback(
    (text: string) => {
      ...
      // Play message sound when user sends a message
      playMessageSound();
      ...
      if (shouldGenerateImage) {
        // Play loading sound when image generation starts
        ...
        // Clear the new panel indicator after animation and play completion sound
        setTimeout(() => {
          setNewlyCreatedPanelId(null);
          // Stop loading sound and play completion sound when image generation finishes
      ...
      setTimeout(() => {
        ...
        // Play message sound when AI response is received
        playMessageSound();
      }, 800);
    },
  ...
  return (
    <div className="h-screen bg-pattern flex flex-col overflow-hidden">
      ...
      <div className="w-full px-4 py-2 flex-1 flex flex-col min-h-0">
        ...
        {/* Eye Toggle Button - Top Right Corner */}
        <Button 
          variant="outline" 
          size="icon" 
        ...
        <main className="flex-1 flex items-center justify-center min-h-0 overflow-hidden p-1" role="main">
          ...
          {/* Control Buttons Panel - positioned to the right of the comic, only when visible */}
          {buttonsVisible && (
            <aside className="fixed top-20 right-4 z-40 flex flex-col gap-3">
              <Dialog>
