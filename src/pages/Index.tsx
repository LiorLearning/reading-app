...
const Index = ({ initialAdventureProps, onBackToPetPage }: IndexProps = {}) => {
  ...
  const images = useMemo(() => [
    typeof rocket1 === 'string' ? rocket1 : rocket1.src,
    typeof spaceport2 === 'string' ? spaceport2 : spaceport2.src,
    typeof alien3 === 'string' ? alien3 : alien3.src,
    typeof cockpit4 === 'string' ? cockpit4 : cockpit4.src
  ], []);
...
Keep tone warm, brief, and curious.`;
  ...
  const initialPanels = useMemo(
    () => [
      { id: crypto.randomUUID(), image: typeof rocket1 === 'string' ? rocket1 : rocket1.src, text: "The brave astronaut climbs into ROCKET!" },
    ],
    []
  );

  const { panels, currentIndex, setCurrent, addPanel, updatePanelImage, redo, reset } = useComic(
    initialPanels.map(panel => ({
      ...panel,
      // Ensure image is a string: already converted in initialPanels
      image: panel.image as string,
    }))
  );
  ...
  const REALTIME_PROVIDER: 'openai' | 'gemini' = (process.env.NEXT_PUBLIC_REALTIME_AUDIO_PROVIDER === 'gemini' ? 'gemini' : 'openai');
  ...
  // OpenAI realtime - temporarily disabled to avoid build issues with @openai/agents
  // The package has compatibility issues with Next.js client-side bundling
  const openaiRT = React.useMemo(() => ({
  ...
  const handleContinueSpecificAdventure = React.useCallback(async (adventureId: string) => {
    ...
    if (targetAdventure) {
      ...
      if (targetAdventure.comicPanels && targetAdventure.comicPanels.length > 0) {
        ...
        const defaultImages = [
          typeof rocket1 === 'string' ? rocket1 : rocket1.src, // imported at top of file
          typeof spaceport2 === 'string' ? spaceport2 : spaceport2.src, // imported at top of file
          typeof alien3 === 'string' ? alien3 : alien3.src, // imported at top of file
        ...
        let initialPanelImage = typeof rocket1 === 'string' ? rocket1 : rocket1.src; // default
              ...
              >
                <div className="flex-1 min-h-0 relative">
                  {(() => {
                    ...
                    const bgImage = (current?.image && typeof current.image === 'string') ? current.image : (typeof rocket1 === 'string' ? rocket1 : rocket1.src);
                    return (
                      <div
                        aria-hidden
      ...
      (async () => {
        try {
          ...
          setChatMessages(prev => {
            ...
            return next;
                ...
                return (
                  ...
                  <ComicPanelComponent
                    image={typeof current.image === 'string' ? current.image : (current.image as any).src || current.image}
          ...
          <div className="mb-2 flex items-center gap-2 text-xs text-blue-600 bg-blue-50 rounded-full px-3 py-1">
            ...
            {process.env.NODE_ENV === 'development' && currentScreen === 1 && (
              <button
                  ...
                  <>
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        router.push('/auth?mode=upgrade&redirect=/app');
                      }}
                    >
