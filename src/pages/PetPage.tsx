  ...
  const handlePreferenceSelection = React.useCallback(async (level: 'start' | 'middle', gradeDisplayName?: string) => {
    ...
    try {
      ...
      if (gradeCode) {
        ...
        // Reset assignment spellbox progress when switching TO assignment
        if (gradeCode === 'assignment' && user?.uid) {
          try {
            await firebaseSpellboxService.deleteSpellboxProgressFirebase(user.uid, 'Assignment');
          } catch (err) {
            // Silent fail
          }
        }
